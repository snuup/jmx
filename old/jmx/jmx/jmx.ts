...
let iselement = (h): h is HElement => typeof h.tag == "string"
...
function sync(p: Element, i: number, h: Expr<H | undefined>, uc: UpdateContext): number {
    ...
    if (isobject(h)) {
        ...
        function syncchildren(p: Element, h: HElement | HComp | HFragment, i: number): number {
            evaluate(h.children)?.flat().forEach(hc => i = sync(p, i, hc, uc))
            return i
        }
        ...
        switch (typeof h.tag) {
            ...
            case 'function':
                ...
                // a component can return undefined or null if it has no
...
export function patch(e: Node, h: Expr<H>, uc: UpdateContext = {}) {
    ...
    let f = () => sync(p, i, h, uc)
    window.setTimeout(f, 0) // always called deferred, because removing elements can trigger events and their handlers (like blur)
}
